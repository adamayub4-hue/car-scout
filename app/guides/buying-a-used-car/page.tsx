...
export default function UsedCarGuide() {
  return <main className="min-h-screen bg-background px-4 py-10 text-foreground"><article className="mx-auto max-w-3xl"><Link href="/guides" className="text-sm font-semibold text-link">← All guides</Link><p className="mt-12 text-xs font-bold uppercase tracking-[0.22em] text-link">Used-car guide</p><h1 className="mt-3 text-4xl font-bold tracking-tight sm:text-5xl">Buying a used car: a practical UK checklist</h1><p className="mt-5 text-lg leading-8 text-muted">A listing is only the start. Work through these checks before committing to a vehicle.</p><div className="mt-10 space-y-8">{sections.map(([title, copy], index) => <section key={title} className="border-t border-outline/10 pt-6"><span className="text-xs font-bold text-link">{String(index + 1).padStart(2, "0")}</span><h2 className="mt-2 text-2xl font-bold">{title}</h2><p className="mt-3 leading-7 text-muted">{copy}</p></section>)}</div><aside className="mt-10 rounded-2xl border border-amber-300/20 bg-amber-300/[0.06] p-5 text-sm leading-6 text-warning">If you are unsure, pay an independent qualified mechanic or inspection service. Mekivo does not inspect vehicles or verify marketplace listings.</aside><Link href="/" className="mt-8 inline-flex rounded-xl bg-sky-400 px-5 py-3 font-bold text-slate-950">Search for a car</Link></article></main>;
}
