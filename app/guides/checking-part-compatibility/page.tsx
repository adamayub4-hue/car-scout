...
export default function CompatibilityGuide() {
  return <main className="min-h-screen bg-[#07101e] px-4 py-10 text-white"><article className="mx-auto max-w-3xl"><Link href="/guides" className="text-sm font-semibold text-sky-300">← All guides</Link><p className="mt-12 text-xs font-bold uppercase tracking-[0.22em] text-violet-300">Compatibility guide</p><h1 className="mt-3 text-4xl font-bold tracking-tight sm:text-5xl">How to check whether a car part fits</h1><p className="mt-5 text-lg leading-8 text-slate-300">Use several matching signals before purchasing. No single photograph or keyword is enough.</p><div className="mt-10 grid gap-4 sm:grid-cols-2">{checks.map(([title, copy]) => <section key={title} className="rounded-2xl border border-white/10 bg-white/[0.035] p-5"><h2 className="text-lg font-bold">{title}</h2><p className="mt-3 text-sm leading-6 text-slate-300">{copy}</p></section>)}</div><section className="mt-10 border-t border-white/10 pt-7"><h2 className="text-2xl font-bold">Before fitting</h2><p className="mt-3 leading-7 text-slate-300">Place the old and replacement parts side by side without forcing or modifying anything. If identifiers, dimensions or connections differ, stop and confirm with the supplier or a qualified mechanic.</p></section><Link href="/" className="mt-8 inline-flex rounded-xl bg-sky-400 px-5 py-3 font-bold text-slate-950">Start a Mekivo search</Link></article></main>;
}
